...
def submit_questionnaire(request):
    if request.method == "POST":
        ...
        data = pd.read_csv('portfolio_recommend/filtered_stock_data_10.csv')

        # 確保正確提取股票代碼和現有的 beta 值及 score 值
        data.rename(columns={'stock': 'stock_code'}, inplace=True)

        # 按照 Beta 值排序
        data = data.sort_values(by='beta', ascending=False)
